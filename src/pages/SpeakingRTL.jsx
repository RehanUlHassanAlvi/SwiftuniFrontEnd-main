...
import Logo from "../assets/images/S_RTL_Logo.svg";
import { S_RTL_Subheading } from "../components/Common/Data";
...
import AiScorePopupSpeakingRTL from "../components/Speaking/AiScorePopupSpeakingRTL";
...
const questionname = "Re-tell Lecture";
...
const SpeakingRTL = () => {
  ...
  const [rtlScore, setRTLScore] = useState("");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (questionsData && recordedWavFile) {
        ...
        const response = await fetch(`${Base_URL_AiScore}/retellecture`, {
          method: "POST",
          body: formData,
        });
        ...
        {scorecardOpen && (
          <Modal isOpen={scorecardOpen} style={modalStyle}>
            <AiScorePopupSpeakingRTL
              ...
              enableSkillsScore={
                myAttemptedQuestionsScore
                  ? JSON.parse(myAttemptedQuestionsScore.UsersResponse).rtlScore
                  : rtlScore
              }
              allData={
                myAttemptedQuestionsScore
        ...
        <FlexDiv style={{ width: "100%" }}>
          ...
          >
            <TestHeadingReTellLec
              logo={Logo}
              heading={"Re-Tell Lecture"}
              subheading={S_RTL_Subheading}
              ...
              reTellLecturePostAudioCountdown={10}
            ...
            {questionsData &&
              ...
              questionsData?.response.QuestionImage && (
                <SWT_QCard_Div>
                  <DescribeImageCard
                    // key={`describe-image-card-component-${resetTrigger}`}
                    id={DescribeImageCardData[0].id}
                    srcImage={questionsData?.response.QuestionImage}
                    onImageLoad={() => setIsDataLoading(false)}
                  />
                </SWT_QCard_Div>
              )}
            ...
            <SWT_MCQCard_Div>
              <SpeakingMicCard
                ...
                isAudioCompleted={isAudioCompleted}
                isReTellLecture={true}
                postAudioPlayTestCountdownTime={10000}
              />
            </SWT_MCQCard_Div>
