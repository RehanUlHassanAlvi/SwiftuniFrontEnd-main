...
import { WEmail_Subheading } from "../components/Common/Data";
import { TextCardReadonlyData } from "../components/Writing/data";
import { AiScoreCardWriteEmailData } from "../components/Writing/data";
...
import AiEmailScorePopup from "../components/Writing/AiEmailScorePopup";
import { useTestQuestions } from "../context/TestQuestionContext";

const questionname = "Write Email";
...
const WriteEmail = () => {
  ...
  const [totalTestTime, setTotalTestTime] = useState(540);
  ...
  const handleSubmit = async () => {
    if (selectedAnswers.trim() !== "" && questionsData) {
      ...
      try {
        ...
        const payload = {
          email: selectedAnswers,
          desc: questionsData?.response.QuestionStatement,
          major_aspect: major_aspect,
          minor_aspect: minor_aspect,
        };

        const response = await fetch(`${Base_URL_AiScore}/email`, {
          method: "POST",
          headers: {
